...
@Builder
@Entity
@Table(name = "details_information", schema = "management_report")
public class DetailInfoEntity implements Serializable {
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "strategy_detail_id", referencedColumnName = "id", foreignKey = @ForeignKey(name = "fk_strategy_detail"))
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "image_id", referencedColumnName = "id", foreignKey = @ForeignKey(name = "fk_image"))
